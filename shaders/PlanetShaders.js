...
void main() {
  ...
  init(srcTextureDay, srcTextureNight) {
    ...
    this.programGrid = compileProgram(
      gl,
      this.vertShaderGrid,
      this.fragShaderGrid
    )
    ...
    this.posAttrLocationGrid = gl.getAttribLocation(
      this.programGrid,
      'a_position'
    )
    this.colorAttrLocationGrid = gl.getAttribLocation(
      this.programGrid,
      'a_color'
    )
    this.matrixLocationGrid = gl.getUniformLocation(
      this.programGrid,
      'u_matrix'
    )
    ...
    const imageLocationNight = gl.getUniformLocation(
      this.program,
      'u_imageNight'
    )
  ...
  draw(viewMatrix, rA, decl, LST, drawTexture, drawGrid, drawMap, rECEF) {
    ...
    const drawTextureLocation = gl.getUniformLocation(
      this.program,
      'u_draw_texture'
    )
  ...
  setGeometryGrid() {
    ...
    for (
      let lat = -nStepLat * latStep;
      lat <= nStepLat * latStep;
      lat += latStep
    ) {
      for (let lon = -180.0; lon < 180.0; lon += lonStep) {
        const xStart =
          gridCoeff * this.a * MathUtils.cosd(lat) * MathUtils.cosd(lon)
        const yStart =
          gridCoeff * this.a * MathUtils.cosd(lat) * MathUtils.sind(lon)
        const zStart = gridCoeff * this.b * MathUtils.sind(lat)
        const xEnd =
          gridCoeff *
          this.a *
          MathUtils.cosd(lat) *
          MathUtils.cosd(lon + lonStep)
        const yEnd =
          gridCoeff *
          this.a *
          MathUtils.cosd(lat) *
          MathUtils.sind(lon + lonStep)
    ...
    for (
      let lon = -nStepLon * lonStep;
      lon <= nStepLon * lonStep;
      lon += lonStep
    ) {
    ...
    for (let indPoly = 0; indPoly < this.polygons.length; indPoly++) {
      ...
      for (let indPoint = 0; indPoint < poly.lon.length - 1; indPoint++) {
        ...
        const xStart =
          gridCoeff *
          this.a *
          MathUtils.cosd(latStart) *
          MathUtils.cosd(lonStart)
        const yStart =
          gridCoeff *
          this.a *
          MathUtils.cosd(latStart) *
          MathUtils.sind(lonStart)
        const zStart = gridCoeff * this.b * MathUtils.sind(latStart)
        const xEnd =
          gridCoeff * this.a * MathUtils.cosd(latEnd) * MathUtils.cosd(lonEnd)
        const yEnd =
          gridCoeff * this.a * MathUtils.cosd(latEnd) * MathUtils.sind(lonEnd)
